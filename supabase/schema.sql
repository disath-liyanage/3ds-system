...
begin
  if not exists (
    ...
    from pg_type t
    join pg_namespace n on n.oid = t.typnamespace
    where t.typname = 'worker_attendance_status'
      and n.nspname = 'public'
  ) then
    create type public.worker_attendance_status as enum ('present', 'absent', 'half_day', 'leave', 'holiday');
  end if;
end $$;
...
create table if not exists public.worker_attendance (
  ...
  note text,
  marked_by uuid references public.users_profile (id),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create table if not exists public.manager_monthly_sales_targets (
  id uuid primary key default gen_random_uuid(),
  manager_id uuid not null references public.users_profile (id) on delete cascade,
  target_month date not null,
  target_amount numeric(12, 2) not null check (target_amount >= 0),
  created_by uuid not null references public.users_profile (id),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (manager_id, target_month)
);
...
create index if not exists idx_manager_monthly_sales_targets_manager_month on public.manager_monthly_sales_targets (manager_id, target_month);
...
alter table public.manager_monthly_sales_targets enable row level security;
...
drop policy if exists "admin_manager_all_manager_monthly_sales_targets" on public.manager_monthly_sales_targets;
...
create policy "admin_manager_all_manager_monthly_sales_targets" on public.manager_monthly_sales_targets
  for all
  using (
    public.current_user_role() in ('admin', 'manager')
  )
